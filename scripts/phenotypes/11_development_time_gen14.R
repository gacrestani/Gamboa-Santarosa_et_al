...
input_df <- read_excel("data/phenotypic_data/raw/Development Time Gen 14.xlsx", skip = 1, col_types = "text")

input_df[] <- lapply(input_df, as.character)

hours <- seq(from = 0, to = 150, by = 6)
...
devtime_boxplots_gen14 <- createPlot()
devtime_boxplots_anc_gen14 <- createPlot(ancestry = TRUE)
